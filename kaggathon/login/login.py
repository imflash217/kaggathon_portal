...
class Login:
    def __init__(
        self,
        password_manager: UsernamePasswordManagerArgon2,
    ...
    def run_and_return_if_access_is_allowed(self) -> bool:
        if (not self.password_manager.is_username_taken(self.session_state.username)) or (
            ...
            is_first_login = self.first_login_checkbox_placeholder.checkbox(
                "🙋🏻‍♂️ This is my first login", value=False
            )
            if is_first_login:
    ...
    def try_login(self) -> bool:
        username = self.username_placeholder.text_input(
            "Username:", value="", max_chars=30
        )
        pwd = self.pwd_placeholder.text_input(
            "Password:", value="", type="password", max_chars=30
        )
    ...
    def try_signup(self) -> bool:
        ...
        pwd2 = self.pwd2_placeholder.text_input(
            "Re-enter password:", value="", type="password", max_chars=30
        )
        ...
        if self.has_user_limit_been_reached():
            st.sidebar.error(
                "🚨 The user limit has been reached. Contact the admin for assistance."
            )
        elif not self._is_valid_username(username):
            st.sidebar.error(
                "❌ Invalid username. Must have only alphanumeric or '.-_ ' characters, "
