...
class UsernamePasswordManagerArgon2:
    ...
    def _create_db(self):
        with sqlite3.connect(self.passwords_db_filepath) as con:
            with closing(con.cursor()) as cursor:
                cursor.execute(
                    "CREATE TABLE IF NOT EXISTS passwords (user TEXT, hash TEXT)"
                )

    def _load_user2hash(self) -> Dict[str, str]:
        with sqlite3.connect(self.passwords_db_filepath) as con:
            with closing(con.cursor()) as cursor:
                user_hash_tuples = cursor.execute(
                    "SELECT user, hash FROM passwords"
                ).fetchall()
        ...
        with sqlite3.connect(self.passwords_db_filepath) as con:
            with closing(con.cursor()) as cursor:
                cursor.execute(
                    ...
                    (username, self.argon2_hasher.hash(password)),
                )
    ...
    def verify(self, username: str, password: str) -> bool:
        ...
        # except (KeyError, argon2.exceptions.VerifyMismatchError):
        except:
            return False
