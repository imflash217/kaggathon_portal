...
class PersonalProgress:
    def __init__(
        self, participant_submissions: SingleParticipantSubmissions, evaluator: Evaluator
    ):
    ...
    def _get_bokeh_progress_plot(self):
        ...
        progress_df = pd.DataFrame(
            [
                [
                    self.participant_submissions.get_submission_name_from_path(
                        submission_filepath
                    ),
                    self.participant_submissions.get_datetime_from_path(
                        submission_filepath
                    ),
                    *[res.value for res in submission_results],
                ]
                for submission_filepath, submission_results in self.participant_submissions.results.items()
            ],
            columns=[
                self.submission_name_column,
                self.submission_time_column,
                *self.metric_names,
            ],
        )
        ...
        return self._create_bokeh_plot_from_df(
            progress_df, self.submission_time_column, self.submission_name_column
        )

    def _create_bokeh_plot_from_df(
        self,
        progress_df: pd.DataFrame,
        submission_time_col: str,
        submission_name_col: str,
    ):
            ...
            p.add_tools(
                HoverTool(
                    tooltips=[
                        (
                            submission_time_col,
                            f"@{{{submission_time_col}}}{{%Y-%m-%d %H:%M}}",
                        ),
