...
from kaggathon.examples.dummy_generate_predictions import GROUND_TRUTH_DATA
...
class ExampleEvaluator(Evaluator):
    ...
    def _evaluate_prediction_dict(
        self, predictions: Dict[str, int]
    ) -> Tuple[Metric, ...]:
        preds_array = np.array(
            [
                predictions.get(k, 1 - self.true_label_dict[k])
                for k in self.true_label_dict.keys()
            ]
        )
        precision, recall, f1, _ = precision_recall_fscore_support(
            y_true=self.labels_array, y_pred=preds_array, average="binary"
        )
